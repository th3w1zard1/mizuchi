...
tmp = Path(sys.argv[1])
base_patterns = [
    ("abs64", "4889f848f7d8480f48c7c3", "x86-64-arg64-abs-cmov-cdecl", "value < 0 ? -value : value", "cmovs"),
    ("neg_if_pos64", "4889f848f7d8480f49c7c3", "x86-64-arg64-neg-if-pos-cmov-cdecl", "value > 0 ? -value : value", "cmovns"),
]
...
for target_format in [None, "macho"]:
    ...
    for name, hex_bytes, rule, expression, cmov in base_patterns:
        patterns.append((f"{prefix}_{name}", hex_bytes, rule, expression, cmov, target_format))

for index, (name, hex_bytes, rule, expression, cmov, target_format) in enumerate(patterns):
    data = bytes.fromhex(hex_bytes)
    address = 0x760000 + index * 0x10
    task = {
        "name": name,
        "address": address,
    ...
    assert "long long value" in sourcegen_candidate["source"], sourcegen_candidate["source"]
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-arg64-abs-cmov-cdecl" and .generationEvidence.cmov == "cmovs")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-arg64-neg-if-pos-cmov-cdecl" and .generationEvidence.cmov == "cmovns")] | length) == 2
