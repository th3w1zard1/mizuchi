...
tmp = Path(sys.argv[1])
base_patterns = [
    ("is_neg", "89f8c1e81fc3", "x86-64-int-signbit-zero-lt-cdecl", "value < 0"),
    ("is_nonneg", "89f8f7d0c1e81fc3", "x86-64-int-signbit-zero-ge-cdecl", "value >= 0"),
]
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-int-signbit-zero-lt-cdecl" and .generationEvidence.operator == "<")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-int-signbit-zero-ge-cdecl" and .generationEvidence.operator == ">=")] | length) == 2
