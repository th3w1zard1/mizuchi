...
tmp = Path(sys.argv[1])
base_patterns = [
    ("udiv2", "89f8d1e8c3", 2, 1, "value / 2u"),
    ("udiv4", "89f8c1e802c3", 4, 2, "value / 4u"),
    ("udiv256", "89f8c1e808c3", 256, 8, "value / 256u"),
]
...
for target_format in [None, "macho"]:
    ...
    for name, hex_bytes, divisor, shift, expression in base_patterns:
        patterns.append((f"{prefix}_{name}", hex_bytes, divisor, shift, expression, target_format))

for index, (name, hex_bytes, divisor, shift, expression, target_format) in enumerate(patterns):
    data = bytes.fromhex(hex_bytes)
    address = 0x670000 + index * 0x10
    ...
    assert sourcegen_candidate["generator"]["rule"] == "x86-64-arg-udiv-pow2-cdecl", sourcegen_candidate
    ...
    assert synthesis_candidates[0].rule == "x86-64-arg-udiv-pow2-cdecl", synthesis_candidates
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-arg-udiv-pow2-cdecl")] | length) == 6 and
  ([.[] | select(.generationEvidence.divisor == 2)] | length) == 2 and
  ([.[] | select(.generationEvidence.divisor == 4)] | length) == 2 and
  ([.[] | select(.generationEvidence.divisor == 256)] | length) == 2
