...
output="$(RECONKIT_PROMPTS_DIR="$prompts_dir" "$SCRIPT" decomp-validate sample_fn)"
grep -q "OK: $prompt_dir/settings.yaml" <<<"$output"
grep -q "CASE_MANIFESTS_OK checked=1" <<<"$output"

all_output="$(RECONKIT_PROMPTS_DIR="$prompts_dir" "$SCRIPT" decomp-validate --all)"
...
if RECONKIT_PROMPTS_DIR="$prompts_dir" "$SCRIPT" decomp-validate sample_fn >/tmp/reconkit-decomp-validate.out 2>/tmp/reconkit-decomp-validate.err; then
  echo "expected decomp-validate to reject invalid case.yaml" >&2
  exit 1
fi
grep -q "unknown status: made_up" /tmp/reconkit-decomp-validate.err
