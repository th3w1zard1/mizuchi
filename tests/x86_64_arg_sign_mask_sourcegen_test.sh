...
tmp = Path(sys.argv[1])
base_patterns = [
    ("sign_mask", "89f8c1f81fc3", "x86-64-arg-sign-mask-cdecl", "value < 0 ? -1 : 0", "mov-eax-edi-sar-eax-31-ret"),
    ("nonsign_mask", "89f8f7d0c1f81fc3", "x86-64-arg-nonsign-mask-cdecl", "value < 0 ? 0 : -1", "mov-eax-edi-not-eax-sar-eax-31-ret"),
]
...
for target_format in [None, "macho"]:
    ...
    for name, hex_bytes, rule, expression, pattern in base_patterns:
        patterns.append((f"{prefix}_{name}", hex_bytes, rule, expression, pattern, target_format))

for index, (name, hex_bytes, rule, expression, pattern, target_format) in enumerate(patterns):
    ...
    assert sourcegen_candidate["generator"]["valueType"] == "int", sourcegen_candidate
    assert sourcegen_candidate["generator"]["returnType"] == "int", sourcegen_candidate
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-arg-sign-mask-cdecl")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-arg-nonsign-mask-cdecl")] | length) == 2 and
  ([.[] | select(.generationEvidence.trueValue == "0xffffffff")] | length) == 4 and
  ([.[] | select(.generationEvidence.falseValue == "0x00000000")] | length) == 4
