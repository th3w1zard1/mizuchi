...
tmp = Path(sys.argv[1])
base_patterns = [
    ("sdiv3", "b856555555f76c240489d0c1e81f01d0", 3, "0x55555556", 32, "value / 3"),
    ("sdiv5", "b867666666f76c240489d0c1e81fd1fa01d0", 5, "0x66666667", 33, "value / 5"),
    ("sdiv10", "b867666666f76c240489d0c1e81fc1fa0201d0", 10, "0x66666667", 34, "value / 10"),
]
...
for index, (name, hex_bytes, convention, divisor, multiplier, shift, expression) in enumerate(patterns):
    ...
    expected_rule = f"stack-arg-sdiv-magic-{convention}"
...
jq -s -e '
  ...
  ([.[] | select(.rule == "stack-arg-sdiv-magic-cdecl")] | length) == 3 and
  ([.[] | select(.rule == "stack-arg-sdiv-magic-stdcall")] | length) == 3 and
