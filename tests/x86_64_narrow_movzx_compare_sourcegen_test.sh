...
tmp = Path(sys.argv[1])
base_patterns = [
    ("u8_lt", "400fb6cf31c083f9070f92c0c3", "x86-64-uint8-lt-movzx-imm8-cdecl", "unsigned int value", "(unsigned char)value < 0x07u"),
    ("u8_ge", "400fb6cf31c083f9070f93c0c3", "x86-64-uint8-ge-movzx-imm8-cdecl", "unsigned int value", "(unsigned char)value >= 0x07u"),
    ("u16_lt", "0fb7cf31c083f9070f92c0c3", "x86-64-uint16-lt-movzx-imm8-cdecl", "unsigned int value", "(unsigned short)value < 0x07u"),
    ("u16_ge", "0fb7cf31c083f9070f93c0c3", "x86-64-uint16-ge-movzx-imm8-cdecl", "unsigned int value", "(unsigned short)value >= 0x07u"),
]
...
for index, (name, hex_bytes, rule, signature_fragment, expression, target_format) in enumerate(patterns):
    ...
    assert sourcegen_candidate["generator"]["scratchRegister"] == "ecx", sourcegen_candidate
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-uint8-lt-movzx-imm8-cdecl")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-uint8-ge-movzx-imm8-cdecl")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-uint16-lt-movzx-imm8-cdecl")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-uint16-ge-movzx-imm8-cdecl")] | length) == 2
