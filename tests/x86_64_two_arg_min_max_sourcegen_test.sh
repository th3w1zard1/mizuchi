...
tmp = Path(sys.argv[1])
base_patterns = [
    ("uint_min", "89f039f70f42c7c3", "x86-64-uint-min-two-args-cdecl", "unsigned int", "a < b ? a : b"),
    ("uint_max", "89f039f70f47c7c3", "x86-64-uint-max-two-args-cdecl", "unsigned int", "a > b ? a : b"),
    ("int_min", "89f039f70f4cc7c3", "x86-64-int-min-two-args-cdecl", "int", "a < b ? a : b"),
    ("int_max", "89f039f70f4fc7c3", "x86-64-int-max-two-args-cdecl", "int", "a > b ? a : b"),
]
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-uint-min-two-args-cdecl")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-uint-max-two-args-cdecl")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-int-min-two-args-cdecl")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-int-max-two-args-cdecl")] | length) == 2
