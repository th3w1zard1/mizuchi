...
tmp = Path(sys.argv[1])
base_patterns = [
    ("udiv3", "b8abaaaaaaf764240489d0d1e8", 3, "0xaaaaaaab", 1, "value / 3u"),
    ("udiv5", "b8cdccccccf764240489d0c1e802", 5, "0xcccccccd", 2, "value / 5u"),
    ("udiv10", "b8cdccccccf764240489d0c1e803", 10, "0xcccccccd", 3, "value / 10u"),
]
...
for index, (name, hex_bytes, convention, divisor, multiplier, shift, expression) in enumerate(patterns):
    ...
    expected_rule = f"stack-arg-udiv-magic-{convention}"
...
jq -s -e '
  ...
  ([.[] | select(.rule == "stack-arg-udiv-magic-cdecl")] | length) == 3 and
  ([.[] | select(.rule == "stack-arg-udiv-magic-stdcall")] | length) == 3 and
