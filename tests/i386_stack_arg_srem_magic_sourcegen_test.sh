...
tmp = Path(sys.argv[1])
base_patterns = [
    ("srem3", "8b4c2404ba5655555589c8f7ea89d0c1e81f01d08d044029c189c8", 3, "0x55555556", 32, "value % 3"),
    ("srem5", "8b4c2404ba6766666689c8f7ea89d0c1e81fd1fa01c28d049229c189c8", 5, "0x66666667", 33, "value % 5"),
    ("srem10", "8b4c2404ba6766666689c8f7ea89d0c1e81fc1fa0201c201d28d049229c189c8", 10, "0x66666667", 34, "value % 10"),
]
...
for index, (name, hex_bytes, convention, divisor, multiplier, shift, expression) in enumerate(patterns):
    ...
    expected_rule = f"stack-arg-srem-magic-{convention}"
...
jq -s -e '
  ...
  ([.[] | select(.rule == "stack-arg-srem-magic-cdecl")] | length) == 3 and
  ([.[] | select(.rule == "stack-arg-srem-magic-stdcall")] | length) == 3 and
