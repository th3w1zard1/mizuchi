...
tmp = Path(sys.argv[1])
base_patterns = [
    ("abs_i", "89f8f7d80f48c7c3", "x86-64-arg-abs-cmov-cdecl", "value < 0 ? -value : value", "cmovs"),
    ("neg_if_pos", "89f8f7d80f49c7c3", "x86-64-arg-neg-if-pos-cmov-cdecl", "value > 0 ? -value : value", "cmovns"),
]
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-arg-abs-cmov-cdecl" and .generationEvidence.cmov == "cmovs")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-arg-neg-if-pos-cmov-cdecl" and .generationEvidence.cmov == "cmovns")] | length) == 2
