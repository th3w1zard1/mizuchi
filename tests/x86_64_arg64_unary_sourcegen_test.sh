...
tmp = Path(sys.argv[1])
base_patterns = [
    ("neg64", "4889f848f7d8c3", "x86-64-arg64-neg-cdecl", "-", "mov-rax-rdi-neg-rax-ret", "return -value;"),
    ("not64", "4889f848f7d0c3", "x86-64-arg64-not-cdecl", "~", "mov-rax-rdi-not-rax-ret", "return ~value;"),
]
...
for target_format in [None, "macho"]:
    ...
    for name, hex_bytes, rule, operator, pattern, expression in base_patterns:
        patterns.append((f"{prefix}_{name}", hex_bytes, rule, operator, pattern, expression, target_format))

for index, (name, hex_bytes, rule, operator, pattern, expression, target_format) in enumerate(patterns):
    data = bytes.fromhex(hex_bytes)
    address = 0x750000 + index * 0x10
    task = {
        "name": name,
        "address": address,
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-arg64-neg-cdecl")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-arg64-not-cdecl")] | length) == 2 and
  ([.[] | select(.generationEvidence.registerArg == "rdi")] | length) == 4
