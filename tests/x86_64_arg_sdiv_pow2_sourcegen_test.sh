...
tmp = Path(sys.argv[1])
base_patterns = [
    ("sdiv2", "89f8c1e81f01f8d1f8c3", 2, 1, 1, "value / 2"),
    ("sdiv4", "8d470385ff0f49c7c1f802c3", 4, 2, 3, "value / 4"),
    ("sdiv8", "8d470785ff0f49c7c1f803c3", 8, 3, 7, "value / 8"),
    ("sdiv16", "8d470f85ff0f49c7c1f804c3", 16, 4, 15, "value / 16"),
]
...
for target_format in [None, "macho"]:
    ...
    for name, hex_bytes, divisor, shift, bias, expression in base_patterns:
        patterns.append((f"{prefix}_{name}", hex_bytes, divisor, shift, bias, expression, target_format))

for index, (name, hex_bytes, divisor, shift, bias, expression, target_format) in enumerate(patterns):
    data = bytes.fromhex(hex_bytes)
    address = 0x7B0000 + index * 0x20
    ...
    assert sourcegen_candidate["generator"]["rule"] == "x86-64-arg-sdiv-pow2-cdecl", sourcegen_candidate
    ...
    assert synthesis_candidates[0].rule == "x86-64-arg-sdiv-pow2-cdecl", synthesis_candidates
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-arg-sdiv-pow2-cdecl")] | length) == 8 and
