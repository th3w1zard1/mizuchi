...
PYTHONPATH="$ROOT/src" python -m mizuchi_re.cli export-context \
  "$TMP_DIR/input" \
  --out-dir "$TMP_DIR/out" \
  --format json \
  --max-files 20 \
...
jq -e '.schema == "mizuchi.llm-context-index.v1"' "$TMP_DIR/out/LLM_CONTEXT.json" >/dev/null
...
grep -q 'Mizuchi LLM Context' "$TMP_DIR/out/LLM_CONTEXT.md"
