...
tmp = Path(sys.argv[1])
base_patterns = [
    ("urem3", "89f889f9baabaaaaaa480fafd148c1ea218d0c5229c8c3", 3, "0xaaaaaaab", 33, "value % 3u"),
    ("urem5", "89f889f9bacdcccccc480fafd148c1ea228d0c9229c8c3", 5, "0xcccccccd", 34, "value % 5u"),
    ("urem10", "89f889f9bacdcccccc480fafd148c1ea2301d28d0c9229c8c3", 10, "0xcccccccd", 35, "value % 10u"),
]
...
for index, (name, hex_bytes, divisor, multiplier, shift, expression, target_format) in enumerate(patterns):
    ...
    assert sourcegen_candidate["generator"]["rule"] == "x86-64-arg-urem-magic-cdecl", sourcegen_candidate
    ...
    assert synthesis_candidates[0].rule == "x86-64-arg-urem-magic-cdecl", synthesis_candidates
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-arg-urem-magic-cdecl")] | length) == 6 and
