...
tmp = Path(sys.argv[1])
base_patterns = [
    ("srem3", "4863c74869c8565555554889ca48c1ea3f48c1e92001d18d0c4929c8c3", 3, "0x55555556", 32, "value % 3"),
    ("srem5", "4863c74869c8676666664889ca48c1ea3f48c1f92101d18d0c8929c8c3", 5, "0x66666667", 33, "value % 5"),
    ("srem10", "4863c74869c8676666664889ca48c1ea3f48c1f92201d101c98d0c8929c8c3", 10, "0x66666667", 34, "value % 10"),
]
...
for index, (name, hex_bytes, divisor, multiplier, shift, expression, target_format) in enumerate(patterns):
    ...
    assert sourcegen_candidate["generator"]["rule"] == "x86-64-arg-srem-magic-cdecl", sourcegen_candidate
    ...
    assert synthesis_candidates[0].rule == "x86-64-arg-srem-magic-cdecl", synthesis_candidates
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-arg-srem-magic-cdecl")] | length) == 6 and
