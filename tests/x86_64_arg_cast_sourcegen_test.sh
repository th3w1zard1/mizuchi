...
tmp = Path(sys.argv[1])
base_patterns = [
    ("u8", "400fb6c7c3", "x86-64-arg-cast-u8-cdecl", "unsigned int value", "(unsigned char)value"),
    ("i8", "400fbec7c3", "x86-64-arg-cast-i8-cdecl", "int value", "(signed char)value"),
    ("u16", "0fb7c7c3", "x86-64-arg-cast-u16-cdecl", "unsigned int value", "(unsigned short)value"),
    ("i16", "0fbfc7c3", "x86-64-arg-cast-i16-cdecl", "int value", "(short)value"),
]
...
for target_format in [None, "macho"]:
    ...
    for suffix, hex_bytes, rule, parameter, expression in base_patterns:
        patterns.append((f"{prefix}_{suffix}", hex_bytes, rule, parameter, expression, target_format))

for index, (name, hex_bytes, rule, parameter, expression, target_format) in enumerate(patterns):
    data = bytes.fromhex(hex_bytes)
    address = 0x550000 + index * 0x10
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-arg-cast-u8-cdecl")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-arg-cast-i8-cdecl")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-arg-cast-u16-cdecl")] | length) == 2 and
  ([.[] | select(.rule == "x86-64-arg-cast-i16-cdecl")] | length) == 2
