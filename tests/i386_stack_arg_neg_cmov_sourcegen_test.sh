...
tmp = Path(sys.argv[1])
base_patterns = [
    ("abs_i", "8b4c240489c8f7d80f48c1", "stack-arg-abs-cmov", "value < 0 ? -value : value", "cmovs"),
    ("neg_if_pos", "8b4c240489c8f7d80f49c1", "stack-arg-neg-if-pos-cmov", "value > 0 ? -value : value", "cmovns"),
]

patterns = []
for name, core_hex, rule_prefix, expression, cmov in base_patterns:
    patterns.append((f"{name}_cdecl", f"{core_hex}c3", f"{rule_prefix}-cdecl", expression, cmov))
    patterns.append((f"{name}_stdcall", f"{core_hex}c20400", f"{rule_prefix}-stdcall", expression, cmov))

tasks = []
for index, (name, hex_bytes, rule, expression, cmov) in enumerate(patterns):
...
jq -s -e '
  ...
  ([.[] | select(.rule == "stack-arg-abs-cmov-cdecl" and .generationEvidence.cmov == "cmovs")] | length) == 1 and
  ([.[] | select(.rule == "stack-arg-abs-cmov-stdcall" and .generationEvidence.cmov == "cmovs")] | length) == 1 and
  ([.[] | select(.rule == "stack-arg-neg-if-pos-cmov-cdecl" and .generationEvidence.cmov == "cmovns")] | length) == 1 and
  ([.[] | select(.rule == "stack-arg-neg-if-pos-cmov-stdcall" and .generationEvidence.cmov == "cmovns")] | length) == 1
