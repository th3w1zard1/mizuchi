...
base_patterns = [
    (
        "choose_255_0",
        "85ffb8ff0000000f44c7c3",
        "value != 0 ? 0x000000ffu : 0x00000000u",
        "cmove",
        "0x000000ff",
        "0x00000000",
    ),
    (
        "choose_0_255",
        "31c985ffb8ff0000000f45c1c3",
        "value != 0 ? 0x00000000u : 0x000000ffu",
        "cmovne",
        "0x00000000",
        "0x000000ff",
    ),
...
for target_format in [None, "macho"]:
    ...
    for name, hex_bytes, expression, cmov, true_value, false_value in base_patterns:
        patterns.append((f"{prefix}_{name}", hex_bytes, expression, cmov, true_value, false_value, target_format))

for index, (name, hex_bytes, expression, cmov, true_value, false_value, target_format) in enumerate(patterns):
    ...
    assert sourcegen_candidate["generator"]["rule"] == "x86-64-arg-nonzero-cmov-const-select-cdecl", sourcegen_candidate
    ...
    assert any(candidate.rule == "x86-64-arg-nonzero-cmov-const-select-cdecl" for candidate in synthesis_candidates), synthesis_candidates
...
jq -s -e '
  ...
  ([.[] | select(.rule == "x86-64-arg-nonzero-cmov-const-select-cdecl")] | length) == 4 and
  ([.[] | select(.generationEvidence.cmov == "cmove")] | length) == 2 and
  ([.[] | select(.generationEvidence.cmov == "cmovne")] | length) == 2 and
  ([.[] | select(.generationEvidence.trueValue == "0x000000ff")] | length) == 2 and
  ([.[] | select(.generationEvidence.falseValue == "0x000000ff")] | length) == 2
