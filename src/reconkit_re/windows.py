...
def run_recovery_windows(
    *,
    ...
    aggregate = {
        "schema": "reconkit.recovery-windows.v1",
        "status": "running",
        "startedAt": now(),
...
def load_resume_summary(summary_path: Path, base_config: RecoveryConfig, window_size: int) -> dict[str, Any]:
    ...
    if not summary:
        return {}
    if summary.get("schema") != "reconkit.recovery-windows.v1":
        return {}
...
def write_window_coverage(base_dir: Path, aggregate: dict[str, Any]) -> dict[str, Any]:
    ...
    coverage = {
        "schema": "reconkit.recovery-window-coverage.v1",
...
def render_coverage_markdown(coverage: dict[str, Any]) -> str:
    ...
    lines = [
        "# ReconstructKit Recovery Coverage",
        "",
...
def run_source_parity_synthesis(
    *,
    ...
    schema = "reconkit.recovery-windows-source-parity-synthesis.v1"
    if not enabled:
...
def promote_source_parity_accepts(source_package: dict[str, Any], source_parity: dict[str, Any]) -> dict[str, Any]:
    schema = "reconkit.recovery-windows-source-parity-promotion.v1"
    ...
    for row in iter_jsonl(accepted_path):
        ...
        metadata = {
            "schema": "reconkit.recovered-source-function.v1",
            "name": row.get("name"),
            "entry": row.get("entry"),
...
def run_source_package_semantic_sweep(
    ...
    *,
    ...
    if not enabled:
        return {"schema": "reconkit.recovery-windows-semantic-sweep.v1", "enabled": False, "status": "disabled"}
    package_dir_value = source_package.get("packageDir")
    if not package_dir_value:
        return {"schema": "reconkit.recovery-windows-semantic-sweep.v1", "enabled": True, "status": "missing-package", "reason": "source package has no packageDir"}
    function_count = int(source_package.get("functionCount") or 0)
    if function_count <= 0:
        return {"schema": "reconkit.recovery-windows-semantic-sweep.v1", "enabled": True, "status": "skipped-no-functions", "reason": "source package contains no generated function candidates"}
    ...
    try:
        ...
        return {
            "schema": "reconkit.recovery-windows-semantic-sweep.v1",
            "enabled": True,
            "status": "failed",
...
def build_recovered_source_package(base_dir: Path, windows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    manifest = {
        "schema": "reconkit.recovered-source-package.v1",
        "status": "complete",
        "packageDir": str(package_dir),
...
def render_source_index(manifest: dict[str, Any]) -> str:
    lines = [
        "# ReconstructKit Recovered Source Package",
        "",
