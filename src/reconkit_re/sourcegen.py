...
def generate_source_candidates(
    *,
    ...
    return {
        "schema": "reconkit.source-generation.v1",
        "status": status,
        "target": target,
...
def nonsemantic_catalog_row(task: dict[str, Any], data: bytes | None, generator_rule: str) -> dict[str, Any]:
    ...
    return {
        "schema": "reconkit.source-generation.nonsemantic-slice.v1",
        "name": task.get("name"),
...
def write_source_coverage_artifacts(
    ...
    *,
    ...
    coverage = {
        "schema": "reconkit.source-generation.semantic-coverage.v1",
        "taskCount": task_count,
        "targetSlices": target_slice_count,
        "generatedSourceCandidates": generated_count,
        "semanticSourceCandidates": semantic_generated_count,
    ...
    return {
        "schema": "reconkit.source-generation.coverage-artifacts.v1",
        "status": "complete",
...
def write_boundary_repair_artifacts(
    ...
    summary = {
        "schema": "reconkit.source-generation.boundary-repair-summary.v1",
        "status": "complete",
    ...
    return {
        "schema": "reconkit.source-generation.boundary-repair-artifacts.v1",
        "status": "complete",
        "fragmentCount": len(rows),
        "appliedRepairCount": len(applied_boundary_repairs),
...
def boundary_repair_manifest_row(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        "schema": "reconkit.source-generation.boundary-repair-row.v1",
        "name": row.get("name"),
        "entry": row.get("entry"),
...
def build_address_alias_metadata(candidates: list[dict[str, Any]]) -> tuple[dict[str, dict[str, Any]], list[dict[str, Any]]]:
    ...
    for address, bucket in by_address.items():
        ...
        group = {
            "schema": "reconkit.address-alias-group.v1",
        ...
        for row in rows:
            ...
            metadata[address_alias_key(row)] = {
                "schema": "reconkit.address-alias.v1",
                "canonicalAddress": f"0x{address:08x}",
                "canonicalName": canonical_name,
                "canonicalEntry": canonical_entry,
                "aliasCount": len(aliases),
...
def write_address_alias_artifacts(out_dir: Path, groups: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    summary = {
        "schema": "reconkit.address-alias-groups.v1",
        "status": "complete",
    ...
    return {
        "schema": "reconkit.address-alias-artifacts.v1",
        "status": "complete",
...
def summarize_generator_opportunities(nonsemantic_catalog: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema": "reconkit.source-generation.generator-opportunities.v1",
        "classes": classes,
...
def write_normalized_function_facts(out_dir: Path, facts: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    summary = {
        "schema": "reconkit.normalized-function-facts.summary.v1",
    ...
    write_json(summary_path, summary)
    return {
        "schema": "reconkit.normalized-function-facts.artifacts.v1",
...
def normalize_function_fact(fact: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": "reconkit.normalized-function-fact.v1",
        "name": fact.get("name"),
...
def load_compiler_profile_artifacts(out_dir: Path, target: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": "reconkit.compiler-profile-artifacts.v1",
...
def apply_tail_fragment_extent_repairs(
    ...
    for index, item in enumerate(rows):
        ...
        repair = {
            "schema": "reconkit.source-generation.applied-boundary-repair.v1",
...
def generated_candidate_from_target_bytes(task: dict[str, Any], data: bytes | None) -> dict[str, Any] | None:
    ...
    generators = [
        ...
        x86_64_three_args_arithmetic64_candidate,
        ...
        x86_64_two_args_absdiff_candidate,
        x86_64_two_args_min_max64_candidate,
        x86_64_two_args_absdiff64_candidate,
...
def x86_64_three_args_arithmetic64_candidate(task: dict[str, Any], data: bytes) -> dict[str, Any] | None:
    if not is_x86_64_task(task):
        return None
    ...
    source = "\n".join(
        [
            "/*",
            f" * Automatically generated from an x86_64 64-bit three-argument {decoded['suffix']} pattern.",
            f" * Target: {task.get('name')} at {task.get('address')}.",
            " * This is an unverified semantic candidate; acceptance requires compiler/object comparison.",
            " */",
...
def x86_64_three_args_bitwise64_candidate(task: dict[str, Any], data: bytes) -> dict[str, Any] | None:
    if not is_x86_64_task(task):
        return None
    ...
    source = "\n".join(
        [
            "/*",
            f" * Automatically generated from an x86_64 64-bit three-argument {decoded['suffix']} bitwise pattern.",
            f" * Target: {task.get('name')} at {task.get('address')}.",
            " * This is an unverified semantic candidate; acceptance requires compiler/object comparison.",
            " */",
            f"unsigned long long {c_name}(unsigned long long a, unsigned long long b, unsigned long long c) {{",
...
def x86_64_three_args_select64_candidate(task: dict[str, Any], data: bytes) -> dict[str, Any] | None:
    if not is_x86_64_task(task):
        return None
    ...
    source = "\n".join(
        [
            "/*",
            f" * Automatically generated from an x86_64 64-bit three-argument {decoded['suffix']} select pattern.",
            f" * Target: {task.get('name')} at {task.get('address')}.",
            " * This is an unverified semantic candidate; acceptance requires compiler/object comparison.",
            " */",
            f"{decoded['valueType']} {c_name}({decoded['valueType']} a, {decoded['valueType']} b, {decoded['valueType']} c) {{",
...
def x86_64_two_args_absdiff_candidate(task: dict[str, Any], data: bytes) -> dict[str, Any] | None:
    if not is_x86_64_task(task):
        return None
    body = strip_alignment_padding(data)
    ...
    source = "\n".join(
        [
            "/*",
            " * Automatically generated from an x86_64 two-argument absolute-difference cmov pattern.",
            f" * Target: {task.get('name')} at {task.get('address')}.",
            " * This is an unverified semantic candidate; acceptance requires compiler/object comparison.",
            " */",
            f"{value_type} {c_name}({value_type} a, {value_type} b) {{",
...
def x86_64_two_args_absdiff64_candidate(task: dict[str, Any], data: bytes) -> dict[str, Any] | None:
    if not is_x86_64_task(task):
        return None
    body = strip_alignment_padding(data)
    ...
    source = "\n".join(
        [
            "/*",
            " * Automatically generated from an x86_64 64-bit two-argument absolute-difference cmov pattern.",
            f" * Target: {task.get('name')} at {task.get('address')}.",
            " * This is an unverified semantic candidate; acceptance requires compiler/object comparison.",
            " */",
            f"unsigned long long {c_name}(unsigned long long a, unsigned long long b) {{",
