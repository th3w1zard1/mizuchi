...
def run_command(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="reconkit-verify-run-") as tmp:
...
def verify_recovered_source_package(
    ...
    *,
    ...
    report = {
        "schema": "reconkit.recovered-source-verification.v1",
...
def compile_with_msvc(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix=f"reconkit-msvc-{stem}-") as tmp:
        work_dir = Path(tmp)
...
def resolve_msvc_root(msvc_root: Path | None) -> Path:
    ...
    return Path("/run/media/brunner56/MyBook/ReconstructKitSource/toolchains/msvc8.0-main")
