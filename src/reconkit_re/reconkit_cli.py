...
def run_decomp_cli_bridge(args: list[str]) -> int:
    ...
    if not decomp.exists():
        print(f"reconkit-cli: missing workspace bridge {decomp}", file=sys.stderr)
        return 1
...
def run_upstream_command(command: str, argv: list[str]) -> int:
    ...
    if command == "atlas":
        if not argv:
            print("Usage: reconkit-cli atlas <prompt-name>", file=sys.stderr)
            return 2
...
def default_work_dir(target_path: Path, preferred_name: str | None = None) -> Path:
    ...
    return Path("target/reconkit-cli") / identity.stable_id
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--work-dir", type=Path, help="Run/state directory. Defaults to target/reconkit-cli/<stable-target-id>.")
    parser.add_argument(
        "--resume",
...
def build_self_check_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="reconkit-cli self-check",
        description="Verify install-time ReconstructKit assets and local recovery tool availability.",
    )
...
def build_upstream_status_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="reconkit-cli upstream-status",
        description="Report how this package maps upstream macabeus/reconkit surfaces.",
    )
    parser.add_argument("--json", action="store_true", help="Emit machine-readable JSON.")
    return parser
...
def upstream_status() -> dict[str, Any]:
    return {
        "schema": "reconkit.upstream-status.v1",
        "upstream": {
            "repository": "https://github.com/macabeus/reconkit",
        ...
        "mappedSurfaces": [
            {
                ...
                "localSurface": "reconkit_re.plugin_pipeline.PluginPipeline",
                "status": "ported",
            },
            {
                "upstreamSurface": "programmatic source/objdiff phase",
                "localSurface": "reconkit_re.source_plugin_runner.run_source_plugin_pipeline",
                "status": "ported-for-source-slices",
            },
            {
                "upstreamSurface": "installable CLI front door",
                "localSurface": "reconkit-cli <binary-or-folder>",
                "status": "adapted-for-binary-recovery",
            },
            {
                "upstreamSurface": "JSON reports",
                "localSurface": "target/reconkit-cli/<target-id>/report.json and stage receipts",
...
def run_upstream_status(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        return 0
    print("Upstream macabeus/reconkit surface mapping")
...
def run_self_check(args: argparse.Namespace) -> int:
    ...
    report = {
        "schema": "reconkit.install-self-check.v1",
...
def run_upstream_command_guard(command: str) -> int:
    print(
        "\n".join(
            [
                f"reconkit-cli: upstream command '{command}' is not packaged in this Python front door.",
                "Use `reconkit-cli upstream-status` for the exact surface mapping.",
                "For direct binary recovery use: `reconkit-cli <path/to/binary-or-folder>`.",
                "The vendored upstream TypeScript implementation remains under `vendor/upstream-reconkit/` in this checkout.",
            ]
        ),
        file=sys.stderr,
    )
