"""Command-line entrypoint for the ReconstructKit recovery orchestrator."""
...
def default_work_dir(target_path: Path) -> Path:
    ...
    return Path("target/reconkit-recover") / identity.stable_id


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="reconkit-recover", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    recover.add_argument("--work-dir", type=Path, help="Run/state directory. Defaults to target/reconkit-recover/<stable-target-id>.")
    ...
    windows.add_argument("--work-dir", type=Path, help="Window run/state directory. Defaults to target/reconkit-recover/<stable-target-id>-windows.")
...
def run_recover_windows(args: argparse.Namespace) -> int:
    if args.force and args.resume:
        raise SystemExit("--force and --resume are mutually exclusive")
    identity = identify_binary(args.input, args.preferred_name)
    work_dir = args.work_dir or Path("target/reconkit-recover") / f"{identity.stable_id}-windows"
    config = RecoveryConfig(
        input_path=args.input,
        work_dir=work_dir,
        preferred_name=args.preferred_name,
