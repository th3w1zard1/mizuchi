...
def run(args: list[str], *, env: dict[str, str] | None = None, timeout: int = 90) -> subprocess.CompletedProcess[str]:
    with tempfile.TemporaryDirectory(prefix="reconkit-run-") as tmp:
...
def source_task_to_queue_row(task: dict[str, Any]) -> dict[str, Any] | None:
    ...
    row = {
        "schema": "reconkit.source-parity-synthesis-row.v1",
        "sourceTask": True,
        ...
        "sourceOrigin": "converted from reconkit source-generation task",
    }
    return row
...
def import_call_self_stdcall(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source = header("import-call-self-stdcall", row) + "\n".join(
        [
            "typedef void (__stdcall *ReconstructKitImportSelf)(void *);",
            "",
            f"void __fastcall {c_name}(void *self) {{",
            f"    (*(ReconstructKitImportSelf *)0x{addr:08x})(self);",
            "}",
            "",
        ]
    )
...
def virtual_call_push_imm_forward_edx(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source = header("virtual-call-push-imm-forward-edx", row) + "\n".join(
        [
            ...
            f"    ((ReconstructKitVMethodArg *)*(void **)self)[{slot}](self, forwarded_edx, {value});",
            "}",
            "",
        ]
    )
...
X86_64_TWO_ARG_ABSDIFF64_OPS: dict[bytes, tuple[str, str]] = {
    b"\x48\x89\xf0\x48\x29\xf8\x48\x29\xf7\x48\x0f\x43\xc7\xc3": (
        "a > b ? a - b : b - a",
        "mov-rax-rsi-sub-rax-rdi-sub-rdi-rsi-cmovae-rax-rdi-ret",
    ),
}


def x86_64_two_args_absdiff64(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    if not is_x86_64_row(row):
        return []
    body = strip_alignment_padding(data)
    decoded = X86_64_TWO_ARG_ABSDIFF64_OPS.get(body)
    if decoded is None:
        return []
    expression, pattern = decoded
    source = header("x86-64-two-args64-absdiff-cdecl", row) + "\n".join(
        [
            f"unsigned long long {c_name}(unsigned long long a, unsigned long long b) {{",
            f"    return {expression};",
            "}",
    ...
    return [
        GeneratedCandidate(
            ...
            variant="sysv-o2-register-args-uint64-absdiff-cmov",
            c_name=c_name,
            symbol=clang_c_symbol(row, c_name),
            source=source,
            callconv="cdecl",
...
def call_indirect_zero(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    direct_source = header("call-indirect-zero", row) + "\n".join(
        [
            "typedef void (__stdcall *ReconstructKitImportIntArg)(unsigned int);",
            "",
            f"void {c_name}(void) {{",
            f"    ReconstructKitImportIntArg *slot = (ReconstructKitImportIntArg *)0x{addr:08x};",
            "    (*slot)(0);",
            "}",
            "",
        ]
    ...
    loaded_source = header("call-indirect-zero", row) + "\n".join(
        [
            ...
            "",
            f"void {c_name}(void) {{",
            f"    ReconstructKitImportIntArg fn = *(ReconstructKitImportIntArg *)0x{addr:08x};",
            "    fn(0);",
            "}",
            "",
        ]
...
def import_call_global_arg(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source = header("import-call-global-arg", row) + "\n".join(
        [
            "typedef void (__stdcall *ReconstructKitImportU32)(unsigned int);",
            "",
            f"void {c_name}(void) {{",
            f"    (*(ReconstructKitImportU32 *)0x{call_addr:08x})(*(unsigned int *)0x{arg_addr:08x});",
            "}",
            "",
        ]
    )
...
def nullable_virtual_call_imm(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source_fastcall = header("nullable-virtual-call-imm", row) + "\n".join(
        [
            "typedef void (__fastcall *ReconstructKitVMethodImm)(void *, int, int);",
            "",
            f"void __fastcall {c_name}(void *self) {{",
            "    void *obj = *(void **)self;",
            "    if (obj != 0) {",
            f"        ((ReconstructKitVMethodImm *)*(void **)obj)[0](obj, 0, {value});",
            "    }",
            "}",
            "",
        ]
    ...
    source_thiscall = header("nullable-virtual-call-imm", row) + "\n".join(
        [
            "typedef void (__thiscall *ReconstructKitVMethodImm)(void *, int);",
            "",
            f"void __fastcall {c_name}(void *self) {{",
            "    void *obj = *(void **)self;",
            "    if (obj != 0) {",
            f"        ((ReconstructKitVMethodImm *)*(void **)obj)[0](obj, {value});",
            "    }",
            "}",
            "",
        ]
...
def virtual_call_eq_global(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source_direct = header("virtual-call-eq-global", row) + "\n".join(
        [
            "typedef unsigned int (__fastcall *ReconstructKitVMethodU32)(void *);",
            "",
            f"unsigned char __fastcall {c_name}(void *self) {{",
            f"    return ((ReconstructKitVMethodU32 *)*(void **)self)[{slot_index}](self) == *(unsigned int *)0x{addr:08x};",
            "}",
            "",
        ]
    )
    source_char_temp = header("virtual-call-eq-global", row) + "\n".join(
        [
            ...
            f"    result = ((ReconstructKitVMethodU32 *)*(void **)self)[{slot_index}](self) == *(unsigned int *)0x{addr:08x};",
            "    return result;",
            "}",
            "",
        ]
...
def bink_buffer_set_direct_draw_forwarder(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    mode_symbol = "_reconkit_global_30068c68"
    direct_draw_symbol = "_reconkit_global_30068c6c"
    surface_symbol = "_reconkit_global_30068c70"
    source = header("bink-buffer-set-direct-draw-forwarder", row) + "\n".join(
        [
            "extern unsigned int reconkit_global_30068c68;",
            "extern void *reconkit_global_30068c6c;",
            "extern void *reconkit_global_30068c70;",
            ...
            "        reconkit_global_30068c6c = direct_draw;",
            "        reconkit_global_30068c70 = surface;",
            "        reconkit_global_30068c68 = 0x08000000u;",
            f"        {refresh_c_name}();",
            "        return 1;",
            "    }",
            "    reconkit_global_30068c6c = 0;",
            "    reconkit_global_30068c70 = 0;",
            "    reconkit_global_30068c68 = 0;",
            "    return 1;",
            "}",
            "",
        ]
...
def stack_arg_range_global_mode_setter(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source = "\n".join(
        [
            "typedef unsigned int reconkit_u32;",
            f"void __cdecl {c_name}(int mode)",
            "{",
            "    switch (mode) {",
            "    case 1:",
            f"        *(volatile reconkit_u32 *)0x{global_address:08x} = 0x{equal_one_value:02x}u;",
            "        return;",
            "    case 2:",
            "    case 3:",
            f"        *(volatile reconkit_u32 *)0x{global_address:08x} = 0x{range_value:02x}u;",
            "        return;",
            "    default:",
            "        return;",
            "    }",
...
def import_call_arg_return_one_stdcall8(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source_direct = header("import-call-arg-return-one-stdcall8", row) + "\n".join(
        [
            "typedef void (__stdcall *ReconstructKitImportArg)(void *);",
            "",
            f"int __stdcall {c_name}(void *arg, int unused) {{",
            "    (void)unused;",
            f"    (*(ReconstructKitImportArg *)0x{addr:08x})(arg);",
            "    return 1;",
            "}",
            "",
        ]
...
def nullable_field_virtual_tailcall_stack8(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source = header("nullable-field-virtual-tailcall-stack8", row) + "\n".join(
        [
            "typedef void (__fastcall *ReconstructKitVMethod)(void *, int, unsigned int, unsigned int);",
            "",
            f"void __fastcall {c_name}(void *self, int forwarded_edx, unsigned int a, unsigned int b) {{",
            f"    void *obj = *(void **)({self_offset(field)});",
            "    if (obj != 0) {",
            f"        ((ReconstructKitVMethod *)*(void **)obj)[{slot_index}](obj, forwarded_edx, a, b);",
            "    }",
            "}",
            "",
        ]
...
def nullable_field_virtual_tailcall_stack4(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source = header("nullable-field-virtual-tailcall-stack4", row) + "\n".join(
        [
            "typedef void (__fastcall *ReconstructKitVMethod)(void *, int, unsigned int);",
            "",
            f"void __fastcall {c_name}(void *self, int forwarded_edx, unsigned int value) {{",
            f"    void *obj = *(void **)({self_offset(field)});",
            "    if (obj != 0) {",
            f"        ((ReconstructKitVMethod *)*(void **)obj)[{slot_index}](obj, forwarded_edx, value);",
            "    }",
            "}",
            "",
        ]
...
def nullable_field_virtual_return_or_zero(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source = header("nullable-field-virtual-return-or-zero", row) + "\n".join(
        [
            ...
            f"    void *obj = *(void **)({self_offset(field)});",
            "    if (obj == 0) return 0;",
            f"    return ((ReconstructKitVMethodU32 *)*(void **)obj)[{slot_index}](obj);",
            "}",
            "",
        ]
    )
...
def field_virtual_tailcall(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source = header("field-virtual-tailcall", row) + "\n".join(
        [
            ...
            f"    void *obj = *(void **)({self_offset(field)});",
            "    if (obj != 0) {",
            f"        ((ReconstructKitVMethod *)*(void **)obj)[{slot_index}](obj);",
            "    }",
            "}",
            "",
        ]
...
def global_virtual_call_stack_arg(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source_value_first = header("global-virtual-call-stack-arg", row) + "\n".join(
        [
            "typedef void (__fastcall *ReconstructKitVMethodArg)(void *, int, unsigned int);",
            "",
            f"void {c_name}(unsigned int value) {{",
            f"    void *obj = *(void **)0x{addr:08x};",
            f"    ((ReconstructKitVMethodArg *)*(void **)obj)[{slot_index}](obj, 0, value);",
            "}",
            "",
        ]
    )
...
def nullable_field_virtual_tailcall_ret(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source = header("nullable-field-virtual-tailcall-ret", row) + "\n".join(
        [
            f"typedef unsigned int (__fastcall *ReconstructKitVMethod)({', '.join(fn_params)});",
            "",
            f"unsigned int __fastcall {c_name}({', '.join(params)}) {{",
            f"    void *obj = *(void **)({self_offset(field)});",
            "    if (obj == 0) return 0;",
            f"    return ((ReconstructKitVMethod *)*(void **)obj)[{slot_index}]({', '.join(call_args)});",
            "}",
            "",
        ]
    )
...
def field_virtual_call_push_zeros(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source = header("field-virtual-call-push-zeros", row) + "\n".join(
        [
            "typedef void (__fastcall *ReconstructKitVMethodZeros)(void *, int, unsigned int, unsigned int, unsigned int, unsigned int);",
            "",
            f"void __fastcall {c_name}(void *self) {{",
            f"    void *obj = *(void **)({self_offset(field)});",
            f"    ((ReconstructKitVMethodZeros *)*(void **)obj)[{slot_index}](obj, 0, 0, 0, 0, 0);",
            "}",
            "",
        ]
    )
...
def nullable_field_virtual_tailcall_ret32(row: dict[str, Any], c_name: str, data: bytes) -> list[GeneratedCandidate]:
    ...
    source = header("nullable-field-virtual-tailcall-ret32", row) + "\n".join(
        [
            f"typedef void (__fastcall *ReconstructKitVMethod)({fn_params});",
            "",
            f"void __fastcall {c_name}({params}) {{",
            f"    void *obj = *(void **)({self_offset(field)});",
            "    if (obj != 0) {",
            f"        ((ReconstructKitVMethod *)*(void **)obj)[{slot_index}]({call_args});",
            "    }",
            "}",
            "",
        ]
...
def attempt_candidate(
    ...
    *,
    ...
    base_record = {
        "schema": "reconkit.source-parity-synthesis-attempt.v1",
        "name": row.get("name"),
        "entry": row.get("entry"),
    ...
    slice_proc = run(
        [
            sys.executable,
            "-m",
            "reconkit_re.swkotor_inventory_slice",
            "--inventory",
            str(inventory),
            "--function",
...
def run_source_shape_search(
    ...
    *,
    ...
    report = {
        "schema": "reconkit.source-shape-search.v1",
...
def semantic_equivalent_variants(row: dict[str, Any], candidate: GeneratedCandidate) -> list[dict[str, Any]]:
        ...
        return [
            {
                ...
                + "\n".join(
                    [
                        "typedef unsigned int reconkit_u32;",
                        f"int __stdcall {c_name}(void *buffer, reconkit_u32 width, reconkit_u32 height) {{",
                        "    char *base;",
                        "    reconkit_u32 source_width;",
                        "    reconkit_u32 source_height;",
                        "    reconkit_u32 flags;",
                        "    reconkit_u32 mask;",
                        "    reconkit_u32 result;",
                        "    if (buffer == 0) {",
                        ...
                        "    result = 1u;",
                        "    if (width == 0u) {",
                        f"        width = *(volatile reconkit_u32 *)0x{global_width:08x};",
                        "    }",
                        "    if (height == 0u) {",
                        f"        height = *(volatile reconkit_u32 *)0x{global_height:08x};",
                        "    }",
                        f"    source_width = *(reconkit_u32 *)(base + 0x{source_width:x});",
                        f"    source_height = *(reconkit_u32 *)(base + 0x{source_height:x});",
                        "    mask = 0u;",
                        ...
                        "        }",
                        "    }",
                        f"    flags = *(reconkit_u32 *)(base + 0x{scale_flags:x});",
                        "    if ((flags & mask) == mask) {",
                        f"        *(reconkit_u32 *)(base + 0x{scaled_width:x}) = width;",
                        "    } else {",
                        "        result = 0u;",
                        "    }",
                        "    mask = 0u;",
                        ...
                        "    }",
                        "    if ((flags & mask) == mask) {",
                        f"        *(reconkit_u32 *)(base + 0x{scaled_height:x}) = height;",
                        "    } else {",
                        "        result = 0u;",
                        "    }",
                        f"    *(reconkit_u32 *)(base + 0x{right:x}) = *(reconkit_u32 *)(base + 0x{scaled_width:x}) + *(reconkit_u32 *)(base + 0x{x_offset:x});",
                        f"    *(reconkit_u32 *)(base + 0x{bottom:x}) = *(reconkit_u32 *)(base + 0x{scaled_height:x}) + *(reconkit_u32 *)(base + 0x{y_offset:x});",
                        "    return (int)result;",
                        "}",
                        "",
                    ]
            ...
            },
            {
                ...
                + "\n".join(
                    [
                        ...
                        "    reconkit_u32 source_height;",
                        "    reconkit_u32 flags;",
                        "    reconkit_u32 horizontal_mask;",
                        "    reconkit_u32 vertical_mask;",
                        "    reconkit_u32 result;",
                        "    if (buffer == 0) {",
                        "        return 0;",
                        "    }",
                        "    base = (char *)buffer;",
                        "    result = 1u;",
                        f"    width = width ? width : *(volatile reconkit_u32 *)0x{global_width:08x};",
                        f"    height = height ? height : *(volatile reconkit_u32 *)0x{global_height:08x};",
                        f"    source_width = *(reconkit_u32 *)(base + 0x{source_width:x});",
                        f"    source_height = *(reconkit_u32 *)(base + 0x{source_height:x});",
                        "    horizontal_mask = 0u;",
                        "    vertical_mask = 0u;",
                        ...
                        "        result = 0u;",
                        "    }",
                        f"    *(reconkit_u32 *)(base + 0x{right:x}) = *(reconkit_u32 *)(base + 0x{x_offset:x}) + *(reconkit_u32 *)(base + 0x{scaled_width:x});",
                        f"    *(reconkit_u32 *)(base + 0x{bottom:x}) = *(reconkit_u32 *)(base + 0x{y_offset:x}) + *(reconkit_u32 *)(base + 0x{scaled_height:x});",
                        "    return (int)result;",
                        "}",
                        "",
                    ]
        ...
        return [
            {
                ...
                + "\n".join(
                    [
                        "typedef unsigned int reconkit_u32;",
                        f"int __cdecl {c_name}(reconkit_u32 *base, int bit) {{",
                        "    int word = bit / 32;",
                        "    int remainder = bit % 32;",
                        "    reconkit_u32 mask = ~((reconkit_u32)-1 << (31 - remainder));",
            ...
            },
            {
                ...
                + "\n".join(
                    [
                        ...
                        "    word = bit / 32;",
                        "    remainder = bit % 32;",
                        "    mask = ~((reconkit_u32)-1 << (31 - remainder));",
                        "    if ((base[word] & mask) != 0u) {",
                        "        return 0;",
                        "    }",
    ...
    if candidate.rule == "rad-aligned-free-forwarder":
        ...
        c_name = safe_c_name(candidate.c_name)
        typedef = "typedef void (__stdcall *reconkit_rad_custom_free)(void *ptr);"
        ...
        return [
            {
                ...
                + "\n".join(
                    [
                        ...
                        "        if (bytes[-2] == 3) {",
                        "            (*(reconkit_rad_custom_free *)(bytes - 8))(base);",
                        "        } else {",
                        f"            {callee}(base);",
                        "        }",
                        "    }",
            ...
            },
            {
                ...
                + "\n".join(
                    [
                        ...
                        "    ptr = bytes - delta;",
                        "    if (bytes[-2] == 3) {",
                        "        (*(reconkit_rad_custom_free *)(bytes - 8))(ptr);",
                        "        return;",
                        "    }",
                        f"    {callee}(ptr);",
                        "}",
            ...
            },
            {
                ...
                + "\n".join(
                    [
                        ...
                        "        delta = bytes[-1];",
                        "        ptr = bytes - delta;",
                        "        (*(volatile reconkit_rad_custom_free *)(bytes - 8))(ptr);",
                        "        return;",
                        "    }",
                        "    delta = bytes[-1];",
                        "    ptr = bytes - delta;",
    ...
    return [
        ...
        },
        {
            ...
            + "\n".join(
                [
                    "struct ReconstructKitGlobals {",
                    "    volatile unsigned int first;",
                    "    volatile unsigned int second;",
                    "};",
                    f"void __stdcall {c_name}(unsigned int first, unsigned int second) {{",
                    f"    struct ReconstructKitGlobals *globals = (struct ReconstructKitGlobals *){first};",
                    "    globals->first = first;",
...
def matched_source_shape_record(record: dict[str, Any], search_path: str) -> dict[str, Any] | None:
    ...
    return {
        "schema": "reconkit.source-shape-code-slice-match.v1",
...
def write_high_level_promotion_targets(
    ...
    *,
    ...
    report = {
        "schema": "reconkit.source-parity.high-level-promotion-targets.v1",
        "status": "complete",
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = {
        "schema": "reconkit.source-parity-synthesis-summary.v1",
