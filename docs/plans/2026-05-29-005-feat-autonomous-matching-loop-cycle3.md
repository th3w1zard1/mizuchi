...
- [x] **U1 Queue Schema:** Queue state survives restart; load/save atomic
- [x] **U2 Scorer:** Functions scored by complexity; score order preserved
- [x] **U3 Matcher:** One-shot Claude invocation; code block parsed correctly
- [x] **U4 Build & Verify:** Build output capped; verified matches committed
- [x] **U5 Vacuum Loop:** Runs autonomously; backoff works; state persists
- [x] **U6 CLI:** User can start/resume/inspect with clear CLI
- [x] **U7 Tests & Docs:** All units tested; docs runnable
- [x] **Integration Test:** End-to-end mock function match succeeds
- [ ] **Manual Smoke Test:** Run vacuum on real prompts for 30 min; verify progress log + queue state

### Delta Update (2026-07-06)

- **Landed:** Cycle 3 vacuum loop scripts (`vacuum.sh`, queue/scorer/matcher libs), CLI surface, operator docs (`vacuum-workflow.md`, `vacuum-architecture.md`, `scorer-extension.md`), `tests/README.md`, `verificationTier` / `acceptanceGate` in `package_verify.py` with unit test, `.autoclaw/orchestrator` manifest + 5-sprint plan for source-parity roadmap, integration test fix for workspace `matched` fixture.
- **Partial:** Real-prompt `vacuum init` seeds 45 prompts (42 pending); 30-minute unattended smoke not run; Phase 1 roadmap doc cross-links not yet updated; Sprint 1 orchestrator assignments not dispatched.
- **Next:** Manual 30-min vacuum smoke on `prompts/`; `/orchestrate assign 1` for remaining Sprint 1 scope; Phase 2 compiler-profile probes per `source-parity-implementation-roadmap.md`.
