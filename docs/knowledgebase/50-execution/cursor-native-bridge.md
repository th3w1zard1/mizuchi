# Cursor-Native Bridge (without ReconstructKit daemon)

When ReconstructKit is not running, use workspace scripts to replicate the **compile → objdiff** loop and the AI sandbox tool **`compile_and_view_assembly`**.
...
| `scripts/lib/reconkit-config.sh` | Read `reconkit.yaml` templates and plugin paths |
...
For a headless runner, set `RECONKIT_MATCHER_COMMAND` to a command that reads
`{{promptFile}}` and writes `{{responseFile}}`; `run-ai-phase.sh` will then run
`matcher.sh` and verify the produced `trial.c`.

Equivalent to ReconstructKit `compile_and_view_assembly`:
...
1. Copy `reconkit.example.yaml` → `reconkit.yaml`
...
`compile-trial.sh` reads `compilerScript` from `reconkit.yaml` when present.

## Context (m2ctx)

- Default header stub: `context/ctx.h`
- ReconstructKit path: `global.getContextScript` in `reconkit.yaml`
- `compile-and-view-assembly.sh` prepends context before compile (matches ReconstructKit concat behavior)
...
- `vendor/m2c` and `vendor/decomp-permuter` optional — set paths in `reconkit.yaml` or env
