 ...
 *
 * Usage: npx tsx src/ui/decomp-atlas/dev-server.ts [-c path/to/reconkit.yaml]
 */
...
async function main() {
  ...
  // Vite plugin to inject window.__RECONKIT_CONFIG__ into the HTML
  const injectConfigPlugin: Plugin = {
    name: 'inject-reconkit-config',
    transformIndexHtml: {
      order: 'pre',
      handler(html) {
        ...
        const script = `<script>window.__RECONKIT_CONFIG__ = ${config};</script>`;
