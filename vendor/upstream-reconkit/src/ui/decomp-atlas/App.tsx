import { ReconstructKitDb } from '@shared/reconkit-db/reconkit-db';
import { useEffect, useState } from 'react';

import { ReconstructKitDbProvider } from './ReconstructKitDbContext';
import { apiClient } from './api-client';
import { Main } from './components/Main';

export function App() {
  const [db, setDb] = useState<ReconstructKitDb | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const config = window.__RECONKIT_CONFIG__;
    if (!config) {
      setError('No ReconstructKit configuration found. Please ensure you are running within the ReconstructKit environment.');
      return;
    }
    ...
    (async () => {
      try {
        ...
        setDb(ReconstructKitDb.fromDump(json.data));
      } catch (err) {
  ...
  const projectName = window.__RECONKIT_CONFIG__?.projectRoot.split(/[\\/]/).pop() || 'Unknown Project';

  return (
    <ReconstructKitDbProvider db={db}>
      <Main projectName={projectName} />
    </ReconstructKitDbProvider>
  );
}
