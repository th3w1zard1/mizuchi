...
export const integratorConfigSchema = z.object({
  ...
  branchTemplate: z.string().default('reconkit/{{functionName}}'),
  ...
  pr: z
    .object({
      ...
      body: z.string().default('Matched `{{functionName}}` via ReconstructKit.'),
    })
    .optional(),
...
export class IntegratorPlugin implements Plugin<IntegratorResult> {
  ...
  #createWorktree(projectRoot: string, branchName: string): { worktreePath: string; branchName: string } {
    const worktreeBase = path.join(os.tmpdir(), 'reconkit-integrator');
  ...
  #runVerifyBuild(worktreePath: string): string {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'reconkit-verify-'));
  ...
  #openPullRequest(worktreePath: string, branchName: string, functionName: string): string {
    ...
    const body = (this.#config.pr?.body ?? 'Matched `{{functionName}}` via ReconstructKit.').replaceAll(
      '{{functionName}}',
      functionName,
    );
