...
function formatToolLabel(name: string, input: Record<string, unknown>): string {
  // Strip MCP server prefix (e.g. "mcp__reconkit__compile_and_view_assembly" → "compile_and_view_assembly")
...
export class ClaudeRunnerPlugin implements Plugin<ClaudeRunnerResult> {
      ...
      ((prompt, options) =>
        query({
          ...
          options: {
            ...
            allowedTools: ['Read', 'Glob', 'Grep', 'mcp__reconkit__compile_and_view_assembly'],
            permissionMode: 'dontAsk',
            cwd: pipelineConfig.projectRoot,
            mcpServers: {
              reconkit: this.#mcpServer,
            },
            debug: config.debug,
            stderr: (data: string) => {
  ...
  /**
   * Create the MCP server for the ReconstructKit tools
   */
