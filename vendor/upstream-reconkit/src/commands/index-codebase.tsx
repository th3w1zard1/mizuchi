...
export const options = z.object({
  config: z
    .string()
    .optional()
    .describe(option({ description: 'Path to reconkit.yaml config file', alias: 'c' })),
  skipEmbeddings: z
    .boolean()
    .optional()
    .default(false)
...
export default function IndexCodebase({ options: opts }: Props) {
  ...
  useEffect(() => {
    async function run() {
      try {
        ...
        setPhase('writing');
        await writeReconstructKitDb(config.projectRoot, indexResult.dump);
    ...
    return (
      <Box flexDirection="column" gap={1}>
        <Text color="green" bold>
          Done! reconkit-db.json written to {projectRoot}/reconkit-db.json
        </Text>
        <Box flexDirection="column">
          <Text>
  ...
  return (
    <Box flexDirection="column" gap={1}>
      <Text bold>ReconstructKit - Index Codebase</Text>
      {projectRoot && (
        <Box flexDirection="column">
