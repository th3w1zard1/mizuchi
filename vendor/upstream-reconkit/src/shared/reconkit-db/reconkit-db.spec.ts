...
import { type DecompFunctionDoc, RECONKIT_DB_VERSION, ReconstructKitDb, type ReconstructKitDbDump } from './reconkit-db';

function makeDump(overrides?: Partial<ReconstructKitDbDump>): ReconstructKitDbDump {
  return {
...
describe('ReconstructKitDb', () => {
  describe('fromDump', () => {
    it('parses a dump correctly', () => {
      const db = ReconstructKitDb.fromDump(makeDump());
      expect(db.functions).toHaveLength(3);
      expect(db.vectors.size).toBe(3);
    });

    it('exposes the platform', () => {
      const db = ReconstructKitDb.fromDump(makeDump({ platform: 'n64' }));
  ...
  describe('getCalledBy', () => {
    ...
    it('returns multiple callers', () => {
      const db = ReconstructKitDb.fromDump(
        makeDump({
          decompFunctions: [
...
describe('ReconstructKitDb difficulty tiers', () => {
