 ...
 * Codebase Indexer
 *
 * Scans a decompilation project to build a ReconstructKitDbDump:
...
import { type DecompFunctionDoc, RECONKIT_DB_VERSION, type ReconstructKitDbDump } from '~/shared/reconkit-db/reconkit-db';
 ...
 * and returns the updated ReconstructKitDbDump.
 */
...
/**
 * Load an existing reconkit-db.json from the project path.
 */
async function loadExistingDb(projectRoot: string): Promise<ReconstructKitDbDump | null> {
  const dbPath = path.join(projectRoot, 'reconkit-db.json');
  try {
    const raw = await fs.readFile(dbPath, 'utf-8');
    const parsed = JSON.parse(raw) as ReconstructKitDbDump;

    if (parsed.version !== RECONKIT_DB_VERSION) {
      throw new Error(`Incompatible ReconstructKit DB version: expected ${RECONKIT_DB_VERSION}, got ${parsed.version}`);
    }
...
/**
 * Write a ReconstructKitDbDump to reconkit-db.json atomically (temp file + rename).
 */
export async function writeReconstructKitDb(projectRoot: string, dump: ReconstructKitDbDump): Promise<void> {
  const dbPath = path.join(projectRoot, 'reconkit-db.json');
