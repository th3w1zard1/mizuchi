...
export class CCompiler {
  ...
  > {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'reconkit-compile-'));
    ...
    try {
      ...
      const combined = contextContent + '\nextern void _RECONKIT_CONCATENATED_CODE();\n' + cCode;
      ...
      if (error instanceof Error) {
        ...
        try {
          ...
          const reconkitConcatenatedMarkerLine =
            1 + preprocessedSource.split('\n').findIndex((line) => line.includes('_RECONKIT_CONCATENATED_CODE'));

          const compilationErrors = formatCompilationError(rawError, reconkitConcatenatedMarkerLine);
          return {
            success: false,
