...
export class DecompPermuter {
  ...
  async run(options: DecompPermuterOptions): Promise<DecompPermuterResult> {
    ...
    try {
      ...
      // Step 2: Write our ReconstructKit-compatible compile.sh
  ...
  async #setupWorkingDir(options: DecompPermuterOptions): Promise<string> {
    const workingDir = await fs.mkdtemp(path.join(os.tmpdir(), 'reconkit-permuter-'));
  ...
  /**
   * Write a ReconstructKit-compatible compile.sh into the permuter working directory.
   *
