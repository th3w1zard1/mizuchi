...
export const pipelineConfigSchema = z.object({
  ...
  mapFilePath: z
    .string()
    .describe('Path to GNU ld map file for resolving function → object file (relative to reconkit.yaml)'),
  target: z.enum(platformTargets).default('gba'),
  nonMatchingAsmFolders: z
    .array(z.string())
    .describe('Directories containing non-matching assembly files (relative to reconkit.yaml)'),
  matchingAsmFolders: z
    .array(z.string())
    .optional()
    .default([])
    .describe('Directories containing matching assembly files (relative to reconkit.yaml)'),
  excludeFromScan: z
    .array(z.string())
    .optional()
    .default(['tools'])
    .describe('Directories to exclude from C source scanning (relative to reconkit.yaml)'),
});

/**
 * The parsed schema fields plus `projectRoot`, which is derived from the
 * directory containing reconkit.yaml (not user-configured).
 */
export type PipelineConfig = z.infer<typeof pipelineConfigSchema> & {
  /** Absolute path to the project root (directory containing reconkit.yaml). Computed, not user-configured. */
  projectRoot: string;
};
...
export function getDefaultConfigPath(cwd: string = process.cwd()): string {
  return path.join(cwd, 'reconkit.yaml');
}
