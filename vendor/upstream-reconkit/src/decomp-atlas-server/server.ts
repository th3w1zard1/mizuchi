...
import { ReconstructKitDb, type ReconstructKitDbDump } from '~/shared/reconkit-db/reconkit-db.js';
...
export function createAtlasServer({ fileConfig, configPath }: { fileConfig: ConfigFile; configPath: string }) {
  ...
  let cachedDb: ReconstructKitDb | null = null;
  ...
  const app = new Hono()
    ...
    .post('/api/loadProject', zValidator('json', z.object({})), async (c) => {
      ...
      const reconkitDbPath = path.join(projectRoot, 'reconkit-db.json');

      let raw: string;
      try {
        raw = await fs.readFile(reconkitDbPath, 'utf-8');
      } catch {
        return c.json({ error: `reconkit-db.json not found at ${reconkitDbPath}` }, 404);
      }

      const dump: ReconstructKitDbDump = JSON.parse(raw);
      cachedDb = ReconstructKitDb.fromDump(dump);
  ...
  app.get('*', async (c) => {
    ...
    const possiblePaths = [
      path.resolve(path.dirname(configPath), 'node_modules/reconkit/src/ui/decomp-atlas/dist/index.html'),
    ...
    for (const htmlPath of possiblePaths) {
      try {
        ...
        const configScript = `<script>window.__RECONKIT_CONFIG__ = ${JSON.stringify({
          serverBaseUrl: '',
