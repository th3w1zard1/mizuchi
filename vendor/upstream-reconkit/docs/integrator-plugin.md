...
Add the `integrator` section under `plugins` in your `reconkit.yaml`:
...
Worktrees are created under the system temp directory (`/tmp/reconkit-integrator/`). On success, the worktree contains the committed (and optionally pushed) changes. On failure, the worktree is left on disk for debugging. Over many runs you may want to clean up stale worktrees:
