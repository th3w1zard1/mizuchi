...
ReconstructKit automates the cycle of writing C code, compiling, and comparing against a target binary, towards the goal of fully automatic **matching decompilation**.
...
```bash
cp reconkit.example.yaml /path/to/you/decomp/project/reconkit.yaml
```

2. **Index your codebase**:

```bash
npm start -- index-codebase --config /path/to/your/decomp/project/reconkit.yaml
```

3. **Start the Decomp Atlas server**:

```bash
npm start -- atlas --config /path/to/your/decomp/project/reconkit.yaml
```
...
```bash
npm start -- run --config /path/to/your/decomp/project/reconkit.yaml
```

## Pipeline Overview

ReconstructKit executes a pipeline of plugins:

![Pipeline Diagram](./media/docs/pipeline-flow.png)

> 📌 **Roadmap**: See the [issues tab](https://github.com/macabeus/reconkit/issues) for planned features.

## Output

ReconstructKit generates three output files:
...
npm start -- atlas --config reconkit.yaml
```

The server reads your `reconkit.yaml` config and serves the Decomp Atlas UI at `http://localhost:3000`.

> Note: Your project must have a `reconkit-db.json` file in the root directory for the Decomp Atlas to work. Generate it with `reconkit index-codebase` (see below).

### Indexing Your Codebase

The `index-codebase` command scans your decompilation project and generates a `reconkit-db.json` file containing all discovered functions, their assembly, C source (if decompiled), call graphs, and vector embeddings.

**1. Configure your `reconkit.yaml`:**
...
# Index the codebase
npm start -- index-codebase --config reconkit.yaml

# Or in development mode
npm run dev -- index-codebase --config reconkit.yaml
```
...
| Flag                    | Description                                              |
| ----------------------- | -------------------------------------------------------- |
| `-c, --config`          | Path to `reconkit.yaml` (defaults to `./reconkit.yaml`)    |
...
**Python requirements for embeddings:** Python 3.10+ is required. On first run, the indexer automatically creates a virtual environment at `~/.cache/reconkit/python-venv/` and installs `torch` and `transformers` (~2-3 GB). The model weights are cached at `~/.cache/huggingface/`. Use `--skip-embeddings` to skip this entirely.
