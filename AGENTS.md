...
Mizuchi is a plugin-based pipeline runner for matching decompilation projects. It automates the cycle of generating C code (via Codex), compiling with agbcc (ARMv4T), and comparing against target binaries using objdiff. The pipeline automatically retries with rich error context when compilation or matching fails.
...
npx vitest run src/plugins/Codex-runner  # Run specific test file/directory
...
```
Prompt Loader → Codex Runner → Compiler → Objdiff
...
- **Codex Runner Plugin** (`src/plugins/Codex-runner/`) - Uses Codex Agent SDK to generate C code, maintains conversation state across retries, caches responses. Provides an MCP tool (`compile_and_view_assembly`) that allows Codex to test-compile code and view assembly before submitting.
...
- **ObjdiffService** (`src/shared/objdiff/`) - Singleton service for objdiff-wasm operations (parsing object files, extracting assembly, comparing symbols). Used by both ObjdiffPlugin and the Codex Runner's MCP tool.
...
The Codex Runner plugin accepts a `queryFactory` parameter for dependency injection in tests.
